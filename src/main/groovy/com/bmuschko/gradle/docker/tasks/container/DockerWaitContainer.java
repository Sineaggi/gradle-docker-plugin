...
public class DockerWaitContainer extends DockerExistingContainer {
    ...
    public void runRemoteCommand() {
        ...
        ResultCallback<WaitResponse> callback = containerCommand.exec(createCallback(getNextHandler()));
        exitCode = awaitStatusTimeout.getOrNull() != null ? ((WaitContainerResultCallback) callback).awaitStatusCode(awaitStatusTimeout.get(), TimeUnit.SECONDS) : ((WaitContainerResultCallback) callback).awaitStatusCode();
        getLogger().quiet("Container exited with code " + String.valueOf(getExitCode()));
    }
