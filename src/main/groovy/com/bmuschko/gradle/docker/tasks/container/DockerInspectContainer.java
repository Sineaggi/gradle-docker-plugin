    ...
    private void defaultResponseHandling() {
        Action<InspectContainerResponse> action = new Action<InspectContainerResponse>() {
            ...
            public void execute(InspectContainerResponse container) {
                ...
                List<String> devices = container.getHostConfig().getDevices() != null ?
                        Arrays.stream(container.getHostConfig().getDevices()).map(it ->
                                it.getPathOnHost() + ":" + it.getPathInContainer() + ":" + it.getcGroupPermissions()
