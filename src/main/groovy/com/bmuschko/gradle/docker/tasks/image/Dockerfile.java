...
public class Dockerfile extends DefaultTask {
    ...
    public Dockerfile() {
        instructions = getProject().getObjects().listProperty(Instruction.class).empty();
        destFile = getProject().getObjects().fileProperty();
        destFile.set(getProject().getLayout().getBuildDirectory().file("docker/Dockerfile"));
        objects = getProject().getObjects();
        fileOperations = ((DefaultProject)getProject()).getFileOperations();
    }
    ...
    private void verifyValidInstructions() {
        ...
        int fromPos = IntStream.range(0, allInstructions.size())
                .filter(idx -> Objects.equals(allInstructions.get(idx).getKeyword(), FromInstruction.KEYWORD))
                .findFirst()
                .orElse(-1);
        int othersPos = IntStream.range(0, allInstructions.size())
                .filter(idx -> !Objects.equals(allInstructions.get(idx).getKeyword(), ArgInstruction.KEYWORD) &&
                        !Objects.equals(allInstructions.get(idx).getKeyword(), FromInstruction.KEYWORD))
                .findFirst()
                .orElse(-1);
    ...
    public void instructionsFromTemplate(java.io.File template) throws IOException {
        ...
        try (Stream<String> lines = Files.lines(template.toPath())) {
            lines.filter(it -> it.length() > 0).forEach((String instruction) -> {
    ...
    public void instructionsFromTemplate(String templatePath) throws IOException {
        instructionsFromTemplate(fileOperations.file(templatePath));
    }
     ...
     * </pre>
     *
     * @param provider Entry point provider
     ...
     * </pre>
     *
     * @param provider Volume provider
     * @see #volume(String...)
     * @since 4.0.0
     */
     ...
     * </pre>
     *
     * @param provider Directory provider
    ...
    public static abstract class StringArrayInstruction implements Instruction {
        ...
        public StringArrayInstruction(String... command) {
            this.commandProvider = Providers.ofNullable(List.of(command));
        }
    ...
    public static abstract class MapInstruction implements Instruction {
        ...
        public String getText() {
            ...
            Map<String, String> evaluatedCommand = commandProvider.getOrNull();

            if (!(evaluatedCommand instanceof Map)) {
                throw new IllegalArgumentException("the given evaluated closure is not a valid input for instruction " + getKeyword() + " while it doesn't provide a `Map` ([ key: value ]) but a `" + evaluatedCommand.getClass() + "` (" + evaluatedCommand + ")");
            }
            commandToJoin = (Map<String, String>) evaluatedCommand;
            ...
            return getKeyword() + " " + joiner.join(commandToJoin);
        }
        ...
        private void validateKeysAreNotBlank(Map<String, String> command) throws IllegalArgumentException {
            command.entrySet().forEach(entry -> {
                if (entry.getKey().trim().length() == 0) {
                    throw new IllegalArgumentException("blank keys for a key=value pair are not allowed: please check instruction " + getKeyword() + " and given pair `" + entry + "`");
                }
            });
        }
    }
     ...
     */
    public static abstract class FileInstruction<T extends Dockerfile.File> implements Instruction {
        ...
        public String getText() {
            ...
            if (fileValue != null) {
                ...
                if (fileValue.getChown() != null) {
                    instruction.append(" ").append("--chown=").append(fileValue.getChown());
                }
                if (fileValue.getSrc() != null && fileValue.getDest() != null) {
                    instruction.append(" ").append(fileValue.getSrc()).append(" ").append(fileValue.getDest());
                }
    ...
    public static class ExposePortInstruction implements Instruction {
        ...
        public ExposePortInstruction(Integer... ports) {
            this.provider = Providers.ofNullable(List.of(ports));
        }
        ...
        public String getText() {
            ...
            if (evaluatedPorts != null && !evaluatedPorts.isEmpty()) {
                return buildText(evaluatedPorts.toArray(Integer[]::new));
            }
        ...
        private String buildText(Integer[] ports) {
            return getKeyword() + " " + Arrays.stream(ports).map(Object::toString).collect(Collectors.joining(" "));
        }
    }
    ...
    public static class EnvironmentVariableInstruction extends MapInstruction {
        ...
        public EnvironmentVariableInstruction(Map envVars) {
            super(envVars);
        }
    ...
    public static class CopyFileInstruction extends FileInstruction<CopyFile> {
        ...
        public String getText() {
            ...
            CopyFile file = getFile();

            if (file != null && file.getStage() != null) {
                int keywordIndex = getKeyword().length();
                text = text.substring(0, keywordIndex) + " --from=" + file.getStage() + text.substring(keywordIndex);
            }
